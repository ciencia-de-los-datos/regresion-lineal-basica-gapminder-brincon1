...
def pregunta_03():
    ...
    reg.fit(X_fertility.to_numpy().reshape(-1, 1), y_life.to_numpy().reshape(-1, 1))

    # Compute las predicciones para el espacio de predicción
    y_pred = reg.predict(prediction_space)

    # Imprima el R^2 del modelo con 4 decimales
    print(reg.score(X_fertility.to_numpy().reshape(-1, 1), y_life.to_numpy().reshape(-1, 1)).round(4))
...
def pregunta_04():
    ...
    linearRegression.fit(X_train.to_numpy().reshape(-1, 1), y_train.to_numpy().reshape(-1, 1))

    # Pronostique y_test usando X_test
    y_pred = linearRegression.predict(X_test.to_numpy().reshape(-1, 1))

    # Compute and print R^2 and RMSE
    print("R^2: {:6.4f}".format(linearRegression.score(X_test.to_numpy().reshape(-1, 1), y_test.to_numpy().reshape(-1, 1))))
